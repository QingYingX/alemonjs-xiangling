...
import { parseTargetArgs, getRawArgs, getEventGroupId, toSafeInteger, parseDurationArg } from '../../model/group/admin.js';
import { isInList } from '../../model/group/lists.js';
import { hasBotGroupAdminPermission, hasBotGroupOwnerPermission, isMasterUser } from '../../model/group/permissions.js';

const unitReg = /^(秒|s|S|分|分钟|m|M|时|小时|h|H|天|日|d|D|周|w|W|月|年|y|Y)$/;
const durationLabel = (args, seconds) => {
    if (!args.length)
        return `${seconds}秒`;
    const last = args[args.length - 1] || '';
    const prev = args[args.length - 2] || '';
    if (unitReg.test(last) && prev)
        return `${prev}${last}`;
...
var adminMute = async () => {
    ...
    const targetIds = mentionedList.data?.length
        ? mentionedList.data.map(item => toSafeInteger(item.UserId)).filter((item) => item !== null)
        : [target.userId];
    const names = [];

...
const formatGroupMemberStats = (groups) => {
    if (!groups.length) {
...
const parseIdAndText = (args) => {
    const [idText, ...messageParts] = args;
    const id = toNumberId(idText);
    const text = messageParts.join(' ').trim();
    return id && text ? { id, text } : null;
};
const getUserAvatarUrl = (userId) => `https://q1.qlogo.cn/g?b=qq&s=640&nk=${userId}`;
const getGroupAvatarUrl = (groupId) => `https://p.qlogo.cn/gh/${groupId}/${groupId}/640`;
const searchGroups = (groups, keyword) => {
...
export { formatFriendList, formatGroupInfo, formatGroupList, formatGroupMemberStats, getGroupAvatarUrl, getUserAvatarUrl, parseIdAndText, searchGroups, toNumberId };

...
export const noticeKeyMap: Record<string, NoticeSettingKey | 'msgSaveDeltime' | 'notificationsAll'> = {
  好友消息: 'privateMessage',
  群消息: 'groupMessage',
...
export const getDefaultNoticeSettings = (): NoticeSettings => {
...
const key = (scope = 'default'): string => createStoreKey('notice-settings', `${scope}.json`);

const read = async (scope = 'default'): Promise<NoticeSettings> => {
...
const write = async (settings: NoticeSettings, scope = 'default'): Promise<void> => {
  await getRedis().set(key(scope), JSON.stringify(settings));
};

export const getNoticeSettings = read;

export const setNoticeSettings = async (settings: Partial<NoticeSettings>, scope = 'default'): Promise<NoticeSettings> => {
  const next = { ...(await read(scope)), ...settings };
  await write(next, scope);
  return next;
};

export const setNoticeSetting = async (name: string, value: boolean | number): Promise<string> => {
  const mapped = noticeKeyMap[name];
  if (!mapped) return `未知通知项：${name}`;
  const settings = await read();
  if (mapped === 'notificationsAll') {
    const enabled = Boolean(value);
    for (const keyName of Object.keys(getDefaultNoticeSettings()) as Array<keyof NoticeSettings>) {
      if (keyName !== 'msgSaveDeltime') settings[keyName] = enabled as never;
    }
  ...
  if (mapped === 'msgSaveDeltime') {
    ...
    if (!Number.isSafeInteger(seconds) || seconds < 120) return '删除缓存时间不能小于120秒。';
...
export const formatNoticeSettings = (settings: NoticeSettings): string => {
  const labelByKey = Object.entries(noticeKeyMap)
    .filter(([, value]) => value !== 'notificationsAll' && value !== 'msgSaveDeltime') as Array<[string, NoticeSettingKey]>;
  return [
    '管理设置通知',

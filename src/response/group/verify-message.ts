...
import { deleteVerifyState, markVerifyKick, type VerifyState } from '../../model/group/verify';
import { getWelcomeConfig, getWelcomeLines } from '../../model/group/welcome';
...
export const getMessageId = (result: unknown): number | null => {
...
const sendGroupMessage = async <T extends EventKeys>(event: Events[T], groupId: number, message: OneBotMessageSegment[]) => {
  const [client] = useClient(event);
  return client.sendGroupMessage({ group_id: groupId, message });
};

export const sendGroupAtText = async <T extends EventKeys>(
  event: Events[T],
  groupId: number,
  userId: number,
  text: string
...
export const sendVerifyPrompt = async <T extends EventKeys>(
  event: Events[T],
  state: VerifyState,
  seconds: number
): Promise<number | null> => {
  const result = await sendGroupAtText(
    event,
    state.groupId,
    state.userId,
    `欢迎！\n请在「${seconds}」秒内发送\n${state.questionText}\n否则将会被移出群聊`
...
export const sendVerifyFailPrompt = async <T extends EventKeys>(event: Events[T], state: VerifyState): Promise<number | null> => {
  const result = await sendGroupAtText(
    event,
    state.groupId,
    state.userId,
...
export const sendVerifySuccess = async <T extends EventKeys>(event: Events[T], groupId: number, userId: number) => {
...
export const recallMsg = async <T extends EventKeys>(event: Events[T], messageId?: number | null): Promise<boolean> => {
...
export const recallVerifyMsgs = async <T extends EventKeys>(event: Events[T], state: VerifyState | null | undefined): Promise<void> => {
  await recallMsg(event, state?.failMsgId);
  await recallMsg(event, state?.remindMsgId);
  await recallMsg(event, state?.verifyMsgId);
};

export const recallLastVerifyPrompt = async <T extends EventKeys>(event: Events[T], state: VerifyState): Promise<VerifyState> => {
  if (state.failMsgId) {
    await recallMsg(event, state.failMsgId);
...
export const isGroupMember = async <T extends EventKeys>(event: Events[T], groupId: number, userId: number): Promise<boolean> => {
  try {
    const [client] = useClient(event);
...
export const finishVerifyWithKick = async <T extends EventKeys>(
  event: Events[T],
  groupId: number,
  userId: number,
  text: string
  ...
  if (!await isGroupMember(event, groupId, userId)) return false;
  const [client] = useClient(event);

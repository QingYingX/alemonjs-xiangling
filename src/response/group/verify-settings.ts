...
import { normalizeGroupMemberInfo, normalizeGroupMemberList } from '../../adapter/onebot';
import { getEventGroupId, toSafeInteger } from '../../model/group/admin';
import { getGroupUserActivity } from '../../model/admin/stats';
import { applyLocalActivity, getNeverSpeakMembers } from '../../model/group/activity';
import { buildVerifyState, deleteVerifyState, getVerifyConfig, getVerifyMode, getVerifyState, isVerifyGroup, saveVerifyState, setVerifyGroup, setVerifyMode, setVerifyTimeout, type VerifyMode } from '../../model/group/verify';
import { finishVerifyWithKick, getMessageId, recallVerifyMsgs, sendGroupAtText, sendVerifyPrompt, sendVerifySuccess } from './verify-message';

type VerifyTimerSet = {
  kick?: ReturnType<typeof setTimeout>;
  remind?: ReturnType<typeof setTimeout>;
};

const verifyTimers = new Map<string, VerifyTimerSet>();

const verifyTimerKey = (groupId: number, userId: number): string => `${groupId}:${userId}`;

export const clearVerifyTimer = (groupId: number, userId: number): void => {
  const key = verifyTimerKey(groupId, userId);
  const timers = verifyTimers.get(key);
  if (!timers) return;
  if (timers.kick) clearTimeout(timers.kick);
  if (timers.remind) clearTimeout(timers.remind);
...
export const startVerify = async (event: ReturnType<typeof useEvent<'message.create' | 'member.add'>>[0]['current'], groupId: number, userId: number) => {
...
const parseTarget = async (): Promise<number | null> => {
...
export default async () => {
  ...
  if (/^\s*[#＃!！]?(切换验证模式|设置验证模式)/.test(text)) {
    ...
    const explicit = text.match(/设置验证模式(计算|提交)/)?.[1] as VerifyMode | undefined;
    const mode: VerifyMode = explicit ?? (await getVerifyMode(groupId) === '提交' ? '计算' : '提交');

...
import { commandEntries, findCommands } from '../../constants/commands';
import { getDataPath } from '../../adapter/storage';
...
const formatCommandExport = (title: string, entries: typeof commandEntries): string => {
  const lines = [
    '香菱指令导出: ' + title,
  ...
  entries.forEach((entry, index) => {
    ...
    if (entry.auth && entry.auth !== 'all') lines.push('   权限: ' + entry.auth);
    lines.push('');
  });
...
const exportCommands = async (args: string[]): Promise<string> => {
  const keyword = args[0] || '全部';
  const entries = keyword === '全部' ? commandEntries : findCommands(keyword);
  if (!entries.length) return '未找到可导出的指令：' + keyword;
...
const formatEntry = (entry: (typeof commandEntries)[number], index: number): string => {

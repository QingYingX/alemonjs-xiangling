...
import { addBannedWord, deleteBannedWord, formatBannedWord, formatBannedWordList, getBannedWordsConfig, matchTypeLabels, parseBannedMode, penaltyTypeLabels, setBannedMuteTime, toggleTitleExactMode, updateTitleWords } from '../../model/group/banned-words';
import { getEventGroupId, getRawArgText, getRawArgs, toSafeInteger } from '../../model/group/admin';

const stripCommand = (text: string, command: RegExp): string => text.replace(command, '').trim();

const parseWordFromAddCommand = (text: string): string => {
  return text.replace(/^\s*[#＃!！]?新增(模糊|精确|正则1|正则2|正则)?(踢黑|踢撤|禁撤|踢|禁|撤)?违禁词/, '').trim();
};

const normalizeRegexWord = (text: string, word: string): string => {
  if (!/正则2/.test(text)) return word;
...
const parseCsvWords = (text: string): string[] => text.split(/[,，]/).map(item => item.trim()).filter(Boolean);

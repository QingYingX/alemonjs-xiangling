import { useEvent, useMention, useMessage, Format } from 'alemonjs';
import { useClient } from '@alemonjs/onebot';
import { getBannedWordsConfig } from '../../model/group/banned-words.js';
import { parseSelfTitleArgs, parseTitleArgs, toSafeInteger } from '../../model/group/admin.js';
import { getGroupMemberRole } from '../../model/group/permissions.js';

const sendText = async (text) => {
    const [message] = useMessage();
    await message.send({ format: Format.create().addText(text) });
};
const isTitleBlocked = async (groupId, title) => {
    const config = await getBannedWordsConfig(groupId);
    if (!config.titleWords.length)
        return false;
...
const resolveBotId = async (client, event) => {

...
import { normalizeGroupMemberList, warnUnsupportedShape } from '../../adapter/onebot.js';
import { getEventGroupId, toSafeInteger } from '../../model/group/admin.js';
import { getGroupUserActivity } from '../../model/admin/stats.js';
import { cancelPendingCleanup, takePendingCleanup, applyLocalActivity, getNeverSpeakMembers, savePendingCleanup, formatNeverSpeakMembers, parseTrailingNumber, formatInactiveRank, formatRecentlyJoined, parseActivityDuration, getInactiveMembers, formatInactiveMembers } from '../../model/group/activity.js';

const send = async (text) => {
    const [message] = useMessage();
    await message.send({ format: Format.create().addText(text) });
};
const kickMembers = async (event, groupId, userIds) => {
    const success = [];
    const failed = [];
...
var activity = async () => {
    ...
    if (!members.length)
        warnUnsupportedShape('getGroupMemberList', rawMembers);
    if (/从未发言/.test(text)) {

import { useEvent, logger, getConfigValue } from 'alemonjs';
import { useClient } from '@alemonjs/onebot';
import { getNoticeSettings } from '../model/admin/notice-settings.js';

const toSegments = (text) => [{ type: 'text', data: { text } }];
const getMasterIds = () => {
    const root = getConfigValue() || {};
    const onebot = root.onebot && typeof root.onebot === 'object' ? root.onebot : {};
...
const buildForwardBody = (current, settings) => {
    ...
    if ((current.name === 'message.create' || current.name === 'private.message.create') && sameId(senderId, selfId))
        return null;
    if (isRecall) {
    ...
    if (current.name === 'private.friend.add') {
        ...
        if (isFriendNotice && !settings.friendNumberChange)
            return null;
        if (!isFriendNotice && !settings.friendRequest)
            return null;
    ...
    if (current.name === 'private.friend.remove') {
        if (!settings.friendNumberChange)
            return null;
        return `[香菱通知] 好友减少\n好友账号：${senderId || '未知'}\n好友昵称：${current.UserName || '未知'}`;
    }
    if (current.name === 'private.guild.add') {
        if (!settings.addGroupApplication && !settings.groupInviteRequest)
            return null;
    ...
    if (current.name === 'member.add') {
        ...
        if (isBot && !settings.groupNumberChange)
            return null;
        if (!isBot && !settings.groupMemberNumberChange)
            return null;
        return isBot
            ? `[香菱通知] 新增群聊\n新增群号：${groupId || '未知'}`
    ...
    if (current.name === 'member.ban' || current.name === 'member.unban') {
        if (!settings.botBeenBanned || !sameId(current.UserId, current.BotId))
            return null;
...
var noticeForwarder = async (_event, next) => {
...
export { noticeForwarder as default };

...
import { extractAtUserIds, extractRawMessageText, recordWhoAt } from '../model/group/who-at';
  ...
  try {
    ...
    if (atUserIds.length && current.GuildId && current.UserId) {
      ...
      const record = {
        ...
        time: Number((current.value as { time?: number } | undefined)?.time) || Math.floor(Date.now() / 1000)
      };

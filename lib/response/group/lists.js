...
import { normalizeGroupList, normalizeGroupMemberList, normalizeGroupMemberInfo } from '../../adapter/onebot.js';
import { getGroupLists, formatList, setWhiteAutoUnban, updateList, isInList } from '../../model/group/lists.js';
...
const parseUserId = (text, rawArgs, mentionedUserId, command) => {
...
const sendNoPermission = async (reason) => {
...
const isGroupAdmin = async () => {
    ...
    if (!groupId || !userId)
        return false;
    const rawRole = String(event.current.value?.sender?.role ?? '');
...
const hasBlacklistPermission = async () => {
    const [event] = useEvent();
    if (event.current.IsMaster)
        return true;
...
const ensureBlacklistPermission = async () => {
    if (await hasBlacklistPermission())
        return true;
    await sendNoPermission('暂无黑名单管理权限，请联系主人使用 #黑名单授权 <QQ> 授权。');
    return false;
};
const ensureWhitelistPermission = async () => {
...
var lists = async () => {
    ...
    if (/^(#|＃|!|！)?(黑|白)名单列表/.test(text.trim())) {
        ...
        if (kind === 'black' && !(await ensureBlacklistPermission()))
            return;
        if (kind === 'white' && !(await ensureWhitelistPermission()))
            return;
        const lists = await getGroupLists();
    ...
    if (/白名单(自动)?解禁/.test(text)) {
        if (!(await ensureWhitelistPermission()))
            return;
    ...
    if (/查黑/.test(text)) {
        if (!(await ensureBlacklistPermission()))
            return;
        const [client] = useClient(event.current);
        ...
        const groupIds = allGroups ? normalizeGroupList(await client.getGroupList()).map(group => Number(group.group_id)).filter(Boolean) : [groupId].filter((item) => Boolean(item));
        if (!groupIds.length) {
        ...
        for (const currentGroupId of groupIds) {
            if (!await hasBotGroupAdminPermission(event.current, currentGroupId)) {
                if (allGroups)
                    summaries.push(`${currentGroupId}: Bot 权限不足`);
                failed++;
                continue;
            }
            ...
            if (allGroups && targets.length)
                summaries.push(`${currentGroupId}: 发现 ${targets.length} 人`);
        }
        if (!found) {
